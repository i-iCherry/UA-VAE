...
if __name__ == "__main__":
    ...
    for epoch in range(param.epoch_count, param.epoch_num + 1):  # outer loop for different epochs
        ...
        if epoch == param.epoch_num_p1 + param.epoch_num_p2 + 1:
            ...
            visualizer.print_phase(model.phase)
        if epoch == param.epoch_num_p1 + param.epoch_num_p2 + param.epoch_num_p3 +1:
            model.phase = 'p4'  # Change to confidnet phase
            visualizer.print_phase(model.phase)
        if epoch == param.epoch_num_p1 + param.epoch_num_p2 + param.epoch_num_p3 + param.epoch_num_p4 +1:
            model.phase = 'p5'  # Change to fine-tuning phase
            visualizer.print_phase(model.phase)
        ...
        if param.save_model:
            ...
            elif epoch % param.save_epoch_freq == 0:  # Save both the generator and the discriminator every
