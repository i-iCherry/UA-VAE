...
class FcSepVaeABC(nn.Module):
    ...
    def __init__(self, omics_dims, norm_layer=nn.BatchNorm1d, leaky_slope=0.2, dropout_p=0, dim_1B=128, dim_2B=1024,
        ...
        # Layer 2
        self.encode_fc_2B = FCBlock(dim_1B * 23, dim_2B, norm_layer=norm_layer, leaky_slope=leaky_slope,
                                    dropout_p=dropout_p,
                                    activation=True)
        ...
        # Layer 3
        self.decode_fc_3B = FCBlock(dim_2B, dim_1B * 23, norm_layer=norm_layer, leaky_slope=leaky_slope,
                                    dropout_p=dropout_p,
                                    activation=True)
    ...
    def encode(self, x):
        ...
        return level_3_A, level_3_B, level_3_C, latent_mean, latent_log_var
    ...
    def forward(self, x):
        level_3_A, level_3_B, level_3_C, mean, log_var = self.encode(x)
        z = self.reparameterize(mean, log_var)
        recon_x = self.decode(z)
        return level_3_A, level_3_B, level_3_C, z, recon_x, mean, log_var
...
class UA_VAE(nn.Module):
    """
    Defines the OmiEmbed combined network
    """

    def __init__(self, net_VAE, net_down, omics_dims, omics_mode='multi_omics', norm_layer=nn.InstanceNorm1d,
                 filter_num=8, kernel_size=9,
...
def define_net(net_VAE, net_down, omics_dims, omics_mode='multi_omics', norm_type='batch', filter_num=8, kernel_size=9,
               leaky_slope=0.2, dropout_p=0, latent_dim=256, class_num=2, time_num=256, task_num=7, init_type='normal',
    ...
    net = UA_VAE(net_VAE, net_down, omics_dims, omics_mode, norm_layer, filter_num, kernel_size, leaky_slope,
                   dropout_p,
...
def define_VAE(net_VAE, omics_dims, omics_mode='multi_omics', norm_type='batch', filter_num=8, kernel_size=9,
