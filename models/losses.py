...
def get_loss_func(loss_name, reduction='mean'):
    ...
    """
    if loss_name == 'BCE':
        return nn.BCEWithLogitsLoss(weight=None, reduction=reduction)
    elif loss_name == 'MSE':
...
def JS_loss(mean, log_var, reduction='mean'):
    pass
...
def MTLR_survival_loss(y_pred, y_true, E, tri_matrix, reduction='mean'):
    ...
    # MTLR loss
    loss = - (torch.sum(torch.log(reduc_phi_censor)) + torch.sum(torch.log(reduc_phi_uncensor)) - torch.sum(
        torch.log(reduc_z_censor)) - torch.sum(torch.log(reduc_z_uncensor)))
